...
if [ -z "${RK_BUILDROOT_CFG_FRAGMENTS}" ]; then
	warning "RK_BUILDROOT_CFG_FRAGMENTS is empty so use ${BUILDROOT_BOARD}_defconfig!"
	make -C "$BUILDROOT_DIR" O="$BUILDROOT_OUTPUT_DIR" ${BUILDROOT_BOARD}_defconfig
else
	warning "RK_BUILDROOT_CFG_FRAGMENTS is not empty so use ${BUILDROOT_BOARD}_defconfig and ${RK_BUILDROOT_CFG_FRAGMENTS}"
	echo "# This is a temporary defconfig generated by the combination of \"${BUILDROOT_BOARD}_defconfig ${RK_BUILDROOT_CFG_FRAGMENTS}\"" > $BUILDROOT_DIR/configs/.tmp_defconfig
	cd $BUILDROOT_DIR/configs && cat  ${BUILDROOT_BOARD}_defconfig ${RK_BUILDROOT_CFG_FRAGMENTS} >> .tmp_defconfig
	make -C "$BUILDROOT_DIR" O="$BUILDROOT_OUTPUT_DIR" .tmp_defconfig
	rm $BUILDROOT_DIR/configs/.tmp_defconfig
fi
