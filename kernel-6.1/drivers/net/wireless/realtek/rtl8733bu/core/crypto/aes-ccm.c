			       ...
			       u8 *x)
{
	...
	aes_encrypt_atk(aes, b, x); /* X_1 = E(K, B_0) */
	...
	aes_encrypt_atk(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
		aes_encrypt_atk(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
